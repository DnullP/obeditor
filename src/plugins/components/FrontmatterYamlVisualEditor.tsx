...
export function FrontmatterYamlVisualEditor(props: FrontmatterYamlVisualEditorProps): ReactNode {
    const t = (key: string, options?: Record<string, unknown>): string =>
        translateEditorTextFromSource(props.capabilities, key, key, options);
    ...
    const showFieldTypeContextMenu = async (
        trigger: Pick<MouseEvent<HTMLElement>, "clientX" | "clientY" | "preventDefault" | "stopPropagation">,
        fieldKey: string,
    ): Promise<void> => {
        const selectedAction = await resolveEditorCapabilities(props.capabilities)?.contextMenu?.show?.(
            [
        ...
        const contextAction = selectedAction as FrontmatterContextAction | null;
        if (!contextAction) {
            return;
        }
        ...
        changeFieldType(fieldKey, contextAction);
    };
    ...
    const openFieldTypeMenu = async (
        ...
        await showFieldTypeContextMenu(event, fieldKey);
    };
