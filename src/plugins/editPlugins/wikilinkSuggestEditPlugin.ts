 ...
 *   - ../../core/capabilities（wikiLinks.suggestTargets / localization）
 *   - @codemirror/state
 *   - @codemirror/view
 *
 * @exports
...
import {
    ...
    type EditorWikiLinkSuggestionItem as WikiLinkSuggestionItem,
} from "../../core/capabilities";
import { registerEditPlugin, type EditPluginContext } from "../editPluginRegistry";
import {
    detectOpenWikiLink,
...
function renderPopupContent(
    ...
    if (items.length === 0) {
        ...
        emptyHint.textContent = translateEditorText(
            capabilities,
            "editorPlugins.noMatchingNote",
            "No matching note",
        );
 ...
 */
function createWikiLinkSuggestExtensions(context: EditPluginContext): Extension[] {
    /** 异步查询计数器，用于丢弃过时的请求结果 */
    let querySeq = 0;
    /** 防抖定时器 */
    ...
    function scheduleQuery(
        ...
        debounceTimer = setTimeout(() => {
            ...
            void suggestEditorWikiLinkTargets(context.capabilities, query, MAX_SUGGESTIONS)
                .then((items) => {
                    // 视图已销毁或请求过时，丢弃结果
...
export function registerWikiLinkSuggestEditPlugin(): () => void {
    return registerEditPlugin({
        id: PLUGIN_ID,
        createExtensions: (context) => createWikiLinkSuggestExtensions(context),
    });
}
