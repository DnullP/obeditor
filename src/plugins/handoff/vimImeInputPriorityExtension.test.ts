/**
 * @module plugins/handoff/vimImeInputPriorityExtension.test
...
describe("handleVimImeTextInput", () => {
    ...
    test("should resolve composing letter keydown from physical code", () => {
        const event = {
            key: "Process",
            code: "KeyJ",
            metaKey: false,
        ...
        expect(resolvePlainTextVimKeydownKey(event, true)).toBe("j");
    });

    test("should route composing keydown through Vim in normal mode", () => {
        const event = {
            key: "Process",
            code: "KeyJ",
            metaKey: false,
        ...
        const cm = createCodeMirrorLike({ insertMode: false });
        const handleKey = mock(() => true);
        const handled = handleVimImeKeydown({
            event,
            view: createViewStub("# Demo", 0),
            vimKey: "j",
            dependencies: {
                getCodeMirror: () => cm,
                handleVimKey: handleKey,
            },
        });
