...
function normalizeYamlText(
    ...
    if (parsedDocument.errors.length > 0) {
        const message = parsedDocument.errors[0]?.message
            ?? translateEditorTextFromSource(capabilities, "frontmatter.yamlError", "Invalid YAML");
...
function saveFrontmatterYaml(
    ...
    if (!isViewAlive(view)) {
        ...
        return {
            ...
            message: translateEditorTextFromSource(
                capabilities,
                "frontmatter.editorClosed",
                "Frontmatter editor is closed",
            ),
        };
    }
    ...
    if (!liveBlock) {
        ...
        return {
            ...
            message: translateEditorTextFromSource(
                capabilities,
                "frontmatter.noFrontmatterBlock",
    ...
    try {
        const normalizedYamlText = normalizeYamlText(rawYamlText, capabilities);
        ...
        try {
            ...
            this.root.render(
                createElement(FrontmatterYamlVisualEditor, {
                    ...
                    onCommitYaml: (yamlText: string) => saveFrontmatterYaml(
                        view,
                        yamlText,
                        this.capabilities,
                    ),
