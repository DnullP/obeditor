 ...
 * @param capabilitiesSource 宿主注入的 WikiLink 能力。
 * @param getCurrentFilePath 获取当前文档路径。
 * @param target wiki 目标文本。
 */
export async function openWikiLinkTarget(
    capabilitiesSource: EditorCapabilitiesSource,
    getCurrentFilePath: () => string,
    target: string,
    options: Pick<WikiLinkNavigationOptions, "getCurrentDocumentContent"> = {},
): Promise<boolean> {
    const currentFilePath = getCurrentFilePath();
    return openEditorWikiLinkTarget(
        resolveEditorCapabilities(capabilitiesSource),
        target,
        {
            currentFilePath,
            currentDocumentContent: options.getCurrentDocumentContent?.(),
        },
    );
}
