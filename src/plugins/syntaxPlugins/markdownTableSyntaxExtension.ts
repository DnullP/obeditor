...
import {
    estimateMarkdownTableWidgetHeight as estimateMarkdownTableWidgetHeightFromModel,
...
export { resolveMarkdownTableEditorWheelDeltaY };
...
function saveMarkdownTable(
    ...
    if (!isViewAlive(view)) {
        ...
        return {
            success: false,
            message: translateEditorTextFromSource(
                capabilities,
                "markdownTable.editorClosed",
                "Markdown table editor is closed",
            ),
        };
    }
    ...
    if (!liveBlock) {
        ...
        return {
            ...
            message: translateEditorTextFromSource(
                capabilities,
                "markdownTable.blockMissing",
                "Markdown table block is missing",
            ),
        };
    }
    ...
    return {
        ...
        message: translateEditorTextFromSource(
            capabilities,
            "markdownTable.synced",
            "Markdown table synced",
        ),
    };
}
...
class MarkdownTableWidget extends WidgetType {
    ...
    constructor(
        ...
        private readonly capabilities: EditorCapabilitiesSource,
    ) {
        super();
        ...
        try {
            ...
            this.root.render(
                createElement(MarkdownTableVisualEditor, {
                    ...
                    onCommitMarkdown: (markdownText: string) => saveMarkdownTable(
                        view,
                        this.blockFrom,
                        markdownText,
                        this.capabilities,
...
function buildMarkdownTableSyntaxState(
    ...
    return {
        blocks,
        decorations: buildMarkdownTableDecorations(
            state,
            blocks,
            getCurrentFilePath,
            capabilities,
...
export function createMarkdownTableSyntaxExtension(
    ...
    const syntaxStateField = StateField.define<MarkdownTableSyntaxState>({
        create(state) {
            return buildMarkdownTableSyntaxState(
                state,
                getCurrentFilePath,
