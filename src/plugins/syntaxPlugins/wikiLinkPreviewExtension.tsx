 ...
 *  - @codemirror/view
 *  - ../../core/capabilities
 *  - ../../components/MarkdownReadView
 *  - ./wikiLinkSyntaxRenderer
 *
 * @example
 *   createWikiLinkPreviewExtension(() => currentFilePath, { capabilities })
 *
 * @exports
...
export interface WikiLinkPreviewExtensionOptions {
    capabilities?: EditorCapabilitiesSource;
...
function buildPreviewCacheKey(currentFilePath: string, target: string): string {
    return `${currentFilePath.replace(/\\/g, "/")}::${target}`;
}
            ...
            <div className="cm-wikilink-preview__body">
                ...
                {props.data.status === "not-found" ? (
                    <div className="cm-wikilink-preview__status">
                        {translateEditorTextFromSource(
                            props.capabilities,
                            "editor.wikilinkPreviewNotFound",
                ...
                {props.data.status === "ready" && props.data.kind === "custom" ? (
                    props.data.render() as ReactNode
                ) : null}
            </div>
        </div>
    );
...
class WikiLinkPreviewPlugin implements PluginValue {
    ...
    private readonly options: WikiLinkPreviewExtensionOptions;
    ...
    constructor(
        ...
        options: WikiLinkPreviewExtensionOptions,
    ) {
        this.view = view;
    ...
    private showPreview(target: WikiLinkPreviewTarget): void {
        ...
        void this.loadPreviewData(currentFilePath, target, requestToken, cacheKey);
    }
    ...
    private async loadPreviewData(
        ...
        try {
            const capabilities = resolveEditorCapabilities(this.options.capabilities);
            const context = {
                currentFilePath,
                currentDocumentContent: this.options.getCurrentDocumentContent?.(),
            };
            const preview = await previewEditorWikiLinkTarget(
                capabilities,
                target.target,
                context,
            );
            ...
            if (preview !== null) {
                const readyData = this.convertCapabilityPreview(preview);
                if (readyData) {
            ...
            const resolved = await resolveEditorWikiLinkTarget(
                capabilities,
                target.target,
                context,
            );
            ...
            const content = resolved.content
                ?? await capabilities?.wikiLinks?.readTargetContent?.(resolved.relativePath);
            if (!this.shouldAcceptAsyncResult(target, requestToken)) {
                return;
            }
            ...
            const readyData: WikiLinkPreviewData = {
                ...
                content,
                revealLine: resolved.revealLine ?? subtarget?.line,
            };
            this.previewCache.set(cacheKey, readyData);
            this.renderPreview(target, readyData);
        } catch (error) {
    ...
    private convertCapabilityPreview(
        preview: EditorWikiLinkPreview,
    ): WikiLinkPreviewData | null {
    ...
    return ViewPlugin.fromClass(
        class extends WikiLinkPreviewPlugin {
            constructor(view: EditorView) {
                super(view, getCurrentFilePath, options);
            }
        },
        {
            eventHandlers: {
