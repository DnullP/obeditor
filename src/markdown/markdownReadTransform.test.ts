...
describe("markdownReadTransform", () => {
    ...
    test("should transform image embeds into read mode media protocols", () => {
        expect(transformMarkdownForReadMode("![[Images/demo.png]]")).toBe(
            "![demo.png](/__obeditor_media_embed__/Images%2Fdemo.png)",
        );
    });
    ...
    test("should preserve image embed layout suffix in read mode media target", () => {
        expect(transformMarkdownForReadMode("![[Images/demo.png|640x360]]")).toBe(
            "![demo.png](/__obeditor_media_embed__/Images%2Fdemo.png%7C640x360)",
        );
    });
    ...
    test("should extract frontmatter and transform enhanced inline syntax", () => {
        ...
        expect(prepared.renderedMarkdown).toBe(
            "\n[mark](/__obeditor_highlight__/mark) [#tag](/__obeditor_tag__/tag) [$E=mc^2$](/__obeditor_inline_latex__/E%3Dmc%5E2)",
        );
    ...
    test("should convert block latex into a dedicated read mode protocol", () => {
        ...
        expect(prepared.renderedMarkdown).toBe(
            "Before\n\n[LaTeX](/__obeditor_block_latex__/a%2Bb)\n\nAfter",
        );
    });
    ...
    test("should decode read mode wikilink href", () => {
        expect(decodeReadModeWikiLinkHref("/__obeditor_wikilink__/Note%2FChild")).toBe("Note/Child");
    ...
    test("should decode read mode custom protocols", () => {
        expect(decodeReadModeMediaEmbedHref("/__obeditor_media_embed__/Images%2Fdemo.png")).toBe("Images/demo.png");
        expect(decodeReadModeMediaEmbedHref("/__ofive_media_embed__/Images%2Fdemo.png")).toBe("Images/demo.png");
        expect(decodeReadModeHighlightHref("/__obeditor_highlight__/mark")).toBe("mark");
        expect(decodeReadModeHighlightHref("/__ofive_highlight__/mark")).toBe("mark");
        expect(decodeReadModeTagHref("/__obeditor_tag__/topic")).toBe("topic");
        expect(decodeReadModeTagHref("/__ofive_tag__/topic")).toBe("topic");
        expect(decodeReadModeInlineLatexHref("/__obeditor_inline_latex__/E%3Dmc%5E2")).toBe("E=mc^2");
        expect(decodeReadModeInlineLatexHref("/__ofive_inline_latex__/E%3Dmc%5E2")).toBe("E=mc^2");
        expect(decodeReadModeBlockLatexHref("/__obeditor_block_latex__/a%2Bb")).toBe("a+b");
