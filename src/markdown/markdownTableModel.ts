...
const MARKDOWN_TABLE_LAYOUT_COMMENT_PATTERN = /^<!--\s*(?:obeditor|ofive)-table-layout:\s*(\{.*\})\s*-->\s*$/;
 ...
 * @description 解析 obeditor 表格布局 HTML 注释；兼容历史 ofive 注释。
...
export function serializeMarkdownTableLayoutComment(layout: MarkdownTableLayout | null | undefined): string | null {
    ...
    return `<!-- obeditor-table-layout: ${JSON.stringify(payload)} -->`;
}
 ...
 * @description 序列化 Markdown 表格，并按需追加 obeditor 表格布局注释。
 * @param model 表格模型。
 * @param layout 布局元数据。
 * @returns Markdown 表格文本。
 */
