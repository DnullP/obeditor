...
describe("markdown table layout metadata", () => {
    test("should parse and serialize table layout comments", () => {
        ...
        expect(comment).toBe('<!-- obeditor-table-layout: {"columns":[164,220],"rows":[34,73]} -->');
        expect(parseMarkdownTableLayoutComment(comment ?? "")).toEqual({
            columnWidths: [164, 220],
            rowHeights: [34, 73],
        });
        expect(parseMarkdownTableLayoutComment(
            '<!-- ofive-table-layout: {"columns":[164],"rows":[34]} -->',
        )).toEqual({
            columnWidths: [164],
            rowHeights: [34],
        });
    ...
    test("should append layout metadata after table markdown", () => {
        expect(serializeMarkdownTableWithLayout(createDefaultMarkdownTableModel(), {
            ...
            "| Cell 1   | Cell 2   |",
            "| Cell 3   | Cell 4   |",
            '<!-- obeditor-table-layout: {"columns":[188,236],"rows":[40,56]} -->',
        ].join("\n"));
    });

    test("should ignore invalid layout comments", () => {
        expect(parseMarkdownTableLayoutComment("<!-- obeditor-table-layout: nope -->")).toBeNull();
